import { ServerErrorPage } from "@/components/modules/server-error-page";
...
const imagesClassNames: Record<number, { container: string, image: string }> = {
    ...
    2: { container: 'md:row-span-3 xl:row-span-4', image: '' },
    3: { container: 'row-span-2 xl:row-span-3', image: '' },
    4: { container: 'row-span-2 xl:row-span-3', image: '' },
    5: { container: 'xl:row-span-6 xl:col-start-3', image: '' },
    6: { container: 'col-span-2 row-span-2 md:row-span-3 md:row-start-6 md:col-start-2 xl:col-start-4 xl:row-start-4 xl:row-span-7', image: '' },
}
...
export default async function OsteopathyPage(): Promise<JSX.Element> {
    ...
    return (
        ...
        >
            <h1 className="absolute w-fit text-nowrap top-[40vh] md:top-1/2 xl:top-1/3 left-1/2 md:left-1/3 xl:left-1/4 -translate-x-1/2 -translate-y-1/2 text-4xl md:text-6xl xl:text-7xl bg-background text-accent z-10 px-8 py-4">{title}</h1>
            {
